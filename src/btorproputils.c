    ...
    Btor *btor, BtorNode *and, BtorBitVector *t, BtorBitVector *s, int32_t eidx)
{
  ...
  if (!btor_is_inv_and (mm, s, t, eidx))
  {
    ...
    Btor *btor, BtorNode *srl, BtorBitVector *t, BtorBitVector *s, int32_t eidx)
{
  ...
  uint32_t i, clz_s, clz_t, shift, sbw;
  BtorNode *e;
  BtorBitVector *res, *bvmax, *tmp;
  BtorMemMgr *mm;
  ...
  /* check invertibility, if not invertible: CONFLICT */
  if (!btor_is_inv_srl (mm, s, t, eidx))
  {
    return res_rec_conf (btor, srl, e, t, s, eidx, cons_srl_bv, ">>");
  }
  ...
  if (eidx)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          /* CONFLICT: shifted bits must match */
          assert (btor_bv_get_bit (s, s->width - 1 - i)
                  == btor_bv_get_bit (t, t->width - 1 - (j + i)));
        }
#endif
        res = btor_bv_uint64_to_bv (mm, (uint64_t) shift, sbw);
  ...
  else
  {
    ...
    assert (btor_bv_get_num_leading_zeros (t) >= shift);
    ...
    Btor *btor, BtorNode *mul, BtorBitVector *t, BtorBitVector *s, int32_t eidx)
{
  ...
  /* check invertibility, if not invertible: CONFLICT */
  if (!btor_is_inv_mul (mm, s, t, eidx))
  {
    return res_rec_conf (btor, mul, e, t, s, eidx, cons_mul_bv, "*");
  }
             ...
             int32_t eidx)
{
  ...
  /* check invertibility, if not invertible: CONFLICT */
  if (!btor_is_inv_udiv (mm, s, t, eidx))
  {
    return res_rec_conf (btor, udiv, e, t, s, eidx, cons_udiv_bv, "/");
  }
